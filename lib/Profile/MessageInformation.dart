...
class MessageInformation extends StatefulWidget {
  ...
  MessageInformation({required this.messageId});

  @override
  _MessageInformationState createState() => _MessageInformationState();
}
...
class _MessageInformationState extends State<MessageInformation>
  ...
  void fetchMessages() async {
    ...
    int initialIndex =
        messages.indexWhere((message) => message.id == widget.messageId);
    ...
    if (initialIndex >= 0 && messages.isNotEmpty) {
      updateReadStatus(currentIndex);
    }
  }

  Future<void> updateReadStatus(int index) async {
    try {
      List<MessageItem> messages = await getMessages();
                    ...
                    onTap: () {
                      setState(() {
                        messages[currentIndex].like =
                            !messages[currentIndex].like;
                        toggleHeartStatus(messages[currentIndex].id,
                            messages[currentIndex].id);
                      });
